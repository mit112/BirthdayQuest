...
struct DataSeeder {
    ...
    // MARK: - Seed Rewards
    //
    // Media is NOT seeded. Every reward ships with an empty `contentUrl` so you can see the
    // key you need to fill in. Until you do, unlocking a reward shows "Content loading soon".
    //
    // To add your own media:
    //   1. Upload to Firebase Storage under  rewards/{rewardId}/{filename}
    //   2. Copy the HTTPS download URL into `contentUrl` below (or edit the Firestore doc directly)
    //   3. For `image` rewards use `contentUrls: [String]` instead of `contentUrl: String`
    //
    // POINTS ECONOMY — read before changing costs:
    //   13 seeded challenges total  715 pts
    //    8 seeded rewards cost      750 pts
    //   So the 8 rewards are deliberately NOT purchasable from the seeded challenges alone.
    //   The remaining ~35+ pts are expected to come from the secret challenges each friend
    //   authors in-app (default 50 pts each, 4 friends = 200 pts), bringing the total to 915.
    //   If you remove the secret-challenge flow, lower the reward costs or raise challenge
    //   values to keep the final badge reachable.
