#!/usr/bin/env python3
"""Regenerate the BirthdayQuest app icon.
...
Writes AppIcon.png and AppIcon-dark.png into the AppIcon.appiconset.
Both are saved as RGB (no alpha channel) — App Store validation rejects
iOS app icons that contain transparency.
"""
...
img.convert('RGB').save(f"{out}/AppIcon.png", 'PNG')
print("Light icon saved!")

dark = ImageEnhance.Brightness(img).enhance(0.7)
dark.convert('RGB').save(f"{out}/AppIcon-dark.png", 'PNG')
